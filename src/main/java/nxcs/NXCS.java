...
//        logger.info(String.format("classify,%s,%f,%f,%f,%f,%d", env.getCurrentLocation(), predictions[0], predictions[1], predictions[2], predictions[3], act));
    ...
    public int classifyUpdate(String state, Point openLocation, double[] moeadWeight, int method) {
        if (state.length() != params.stateLength)
            throw new IllegalArgumentException(
                    String.format("The given state (%s) is not of the correct length", state));
        //TODO: state match and weight match
...
//                .collect(Collectors.toList());
        ...
        List<Classifier> matchSet = filterSetConditionWeight(state, openLocation, method, moeadWeight);
//        double[] chebyPA = generateChebyPredictions(matchSet, moeadWeight);

        double[] predictions0 = generatePredictions(matchSet, 0);
        double[] predictions1 = generatePredictions(matchSet, 1);
...
//            if(predictions1_nor[i]>3){
//                System.out.println(">03");
//                c1++;
//            }
//            if(predictions1_nor[i]>2){
//                System.out.println(">02");
//                c2++;
//            }
//            if(predictions1_nor[i]>1){
...
//            if(predictions1_nor[i]>5){
//                System.out.println(">03");
//                c4++;
//            }
//            if(predictions1_nor[i]>4){
//                System.out.println(">02");
//                c5++;
//            }
//            if(predictions1_nor[i]>3){
...
//            }
        ...
        double[] max0 = new double[4];
        double[] max1 = new double[4];
        for(int i=0;i<4;i++){
            max0[i]=moeadWeight[0]* Math.abs(predictions0_nor[i]-moead.idealpoint[0]);
            max1[i]=moeadWeight[1]* Math.abs(predictions1_nor[i]-moead.idealpoint[1]);
        }

//        double[] p_norm0 = new double[4];
//        double[] p_norm1 = new double[4];
        //normalisation
//        for (int i = 0; i < predictions0.length; i++) {
//            p_norm0[i] = stepNor(max0[i], 100);
//        }
//        for (int i = 0; i < predictions1.length; i++) {
//            p_norm1[i] = rewardNor(max1[i], 1000, 0);
//        }


        int maxObj0 = getMaxIndex(max0);
        int maxObj1 = getMaxIndex(max1);

        return maxObj0<maxObj1? maxObj0:maxObj1;
...
//		}
    ...
    private List<Classifier> updateSet(String previousState, String currentState, Point previousPoint, Point currentPoint, int action, ActionPareto reward, double[] moeadWeight, int groupSize) {
        ...
        if (env.isEndOfProblem(currentState)) {
            ...
            //get max weighted-sum
//            double[] paretoPA = generateTotalPredictions_Norm(setM, moeadWeight);

            //TODO:RETURN PA[0] and PA[1]
            double[] chebyPA = generateChebyPredictions(setM, moeadWeight);
...
//            double[] PA1 = generatePredictions(setM, 1);
            ...
            double Qplus1 = 1 + params.gamma * chebyPA[0];
            ...
            P[1] = reward.getPareto().get(1) + params.gamma * chebyPA[1];
...
//            generateCoveringClassifierbyWeight(previousState, moeadWeight);
    ...
    public double[] generateChebyPredictions(List<Classifier> setM, double[] weight){
        double[] predictions0 = generatePredictions(setM, 0);
        double[] predictions1 = generatePredictions(setM, 1);
...
//                System.out.println(">01");
        ...
        double[] max1 = new double[4];
        for(int i=0;i<4;i++){
            max0[i]=weight[0]* Math.abs(predictions0_nor[i]-moead.idealpoint[0]);
            max1[i]=weight[1]* Math.abs(predictions1_nor[i]-moead.idealpoint[1]);
        }
...
//        }
        ...
        return max0[maxObj0]<max1[maxObj1]?new double[] {predictions0[maxObj0],predictions1[maxObj0]}:new double[] {predictions0[maxObj1],predictions1[maxObj1]};
    }
